...
int main(int argc, char *argv[])
{
	...
	char buffer[150000],decryptedText[100000],identifier[100];//key[100000],encryptText[150000];
	char* key;
	char* decryptText;
	struct sockaddr_in serverAddress, clientAddress;
	memset(buffer, '\0', 150000);
	memset(decryptedText, '\0', 100000);
	memset(decryptedText, '\0', 100);
...
while(1)
{
	...
	{	
		...
		if (charsRead < 0){fprintf(stderr,"ERROR 1 reading from socket");}

		//Step 8 - (Child) Send the server identifier
		charsRead = send(establishedConnectionFD, "decryption", 10, 0); // Write to the client
		if (charsRead < 0){fprintf(stderr,"ERROR writing to socket");}

		//Step 9 - (Child) Check client identifier
		/*if(strcmp(identifier,"encryption")==0) // Invalid connection attempt from otp_enc
		{	
		close(establishedConnectionFD); // Close the existing socket which is connected to the client
		close(listenSocketFD); // Close the listening socket
		//{fprintf(stderr,"Invalid connection attempt - otp_enc cannot use otp_dec_d");}
		}*/


		//Step 10 - (Child) Get the encrypted text, key, and any filler text up to 150000 characters from client.
		charsRead = recv(establishedConnectionFD, buffer, 150000, MSG_WAITALL); // Read the client's message from the socket
		if (charsRead < 0){fprintf(stderr,"ERROR 2 reading from socket");}

		
		//Step 11 - (Child) Parse encrypted text and key from 150k character buffer
		decryptText = strtok(buffer,"@");
		key=strtok(NULL,"@");
			
		//Step 12 - (Child) Confirm Key is long enough
		lengthText=strlen(decryptText);
		lengthKey=strlen(key);
		if(lengthKey<lengthText){fprintf(stderr,"key is too short %d",lengthKey,lengthText);}

		//Step 13 - (Child) Decrypt Text using Key
		int i,keyHolder,textHolder,decryptedHolder;
		memset(decryptedText, '\0', 100000);
		for(i=0;i<lengthText;i++) // Account for case where key is longer than text
		{
		keyHolder=key[i];
		textHolder=decryptText[i];
		...
		decryptedHolder=(((textHolder-keyHolder+27)%27)+16);
	
		//Correct ASCII Value
		if(decryptedHolder==16){decryptedText[i]=-16+'0';}
		else{decryptedText[i]=decryptedHolder + '0';}
		}
		decryptedText[lengthText]='\0';


		//Step 14 - (Child) Send Ecrypted Text to client plus filler up to 100000 characters to close out MSG_WAITALL flag
		int charsWritten=0;
		charsWritten = send(establishedConnectionFD, decryptedText, strlen(decryptedText), 0); 
		char delimiter[1];
		memset(delimiter,'@',1);
