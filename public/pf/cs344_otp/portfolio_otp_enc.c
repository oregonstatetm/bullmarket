...
/* opt_enc.c
Client function which communicates with otp_enc_d
...
int main(int argc, char *argv[])
{
	...
	char* encryptedText; // pointer needed for strtok to parse encrypted text after receiving response from otp_enc_d
	...
	charsWritten = send(socketFD, "encryption", 10, 0); // Write to the server
	...
	if(strcmp(buffer,"decryption")==0) // If the decryption identifier is returned, this is an invalid attempt to connect to otp_dec_d
	{
	...
	//Step 14 - Parse Response and send encryptedText to stdout.
	encryptedText = strtok(buffer,"@");
	printf("%s\n", encryptedText);
